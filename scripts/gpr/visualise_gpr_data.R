...
amplitude <- ggplot(gpr, aes(x = amplitude_dB, y = elevation_m_asl, fill = elevation_m_asl)) +
  geom_density_ridges(alpha= alp3, scale = scl, linewidth=lwd)+
  scale_fill_manual(values = cols2, breaks = c(2600, 2400, 2200, 2000))+
  scale_y_discrete(expand = expansion(mult = c(0.1, 3.1)))+
...
model3 <- lm(log(root_to_soil_ratio) ~ amplitude_dB, data = gpr)
...
pred_model3 <- ggpredict(model3, terms = "amplitude_dB")
...
ggplot(aes(x = amplitude_dB, y = root_to_soil_ratio), data = gpr) +
  geom_point() +
